...
export function executeMemoryWrite(input: Record<string, unknown>): string {
  const { filename, name, description, type, content } = input as {
    filename: string; name: string; description: string; type: string; content: string
  }
